...
func reconstructStreamingResponse(path string, raw []byte, defaults streamResponseDefaults) ([]byte, bool) {
	...
	case "/v1/chat/completions":
		...
		parseSSE(raw, builder.OnJSONEvent)
...
func parseSSE(raw []byte, onJSON func(map[string]any)) {
	for len(raw) > 0 {
...
func parseSSEJSON(event []byte) (map[string]any, bool) {
...
func scanSSEBoundary(data []byte) (idx int, sepLen int) {
...
func scanDataLine(line []byte) ([]byte, bool) {
