...
func (b *responsesStreamCacheBuilder) OnJSONEvent(event map[string]any) {
	...
	case "response.created", "response.completed", "response.failed", "response.incomplete", "response.done":
		...
		b.captureResponseMeta(response)
	...
	case "response.reasoning_text.delta":
		...
		index, ok := b.lookupIndex(event)
		if !ok {
			index = b.ensureReasonIndex(outputIndex, hasOutputIndex)
		}
		b.rememberLocator(event, index)
		b.ReasoningIndex = index
		b.HasReasoning = true
...
func (b *responsesStreamCacheBuilder) captureResponseMeta(response map[string]any) {
...
func (b *responsesStreamCacheBuilder) lookupIndex(event map[string]any) (int, bool) {
...
func (b *responsesStreamCacheBuilder) rememberLocator(event map[string]any, index int) {
	itemID, _ := event["item_id"].(string)
	if itemID == "" {
		return
	}
...
func (b *responsesStreamCacheBuilder) ensureReasonIndex(outputIndex int, hasOutputIndex bool) int {
	if hasOutputIndex {
