...
func (s *qdrantStore) ensureCollection(ctx context.Context, dim int) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.vectorSize == dim && s.indexesReady {
		return nil
	}
	...
	case http.StatusOK:
		...
		s.vectorSize = dim
		return s.ensurePayloadIndexesLocked(ctx)
...
func (s *qdrantStore) ensurePayloadIndexesLocked(ctx context.Context) error {
	indexes := []struct {
		field  string
		schema string
	}{
		{field: "params_hash", schema: "keyword"},
		{field: "expires_at", schema: "integer"},
	}
	for _, index := range indexes {
		...
		resp, err := s.req(ctx, http.MethodPut, "/collections/"+s.collection+"/index", body)
		if err != nil {
			return err
		}
		raw, _ := io.ReadAll(resp.Body)
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			return fmt.Errorf("vecstore qdrant: create payload index %q: %s: %s", index.field, resp.Status, string(raw))
		}
	}
	s.indexesReady = true
	return nil
...
func (s *qdrantStore) Insert(ctx context.Context, key string, vec []float32, response []byte, paramsHash string, promptText string, ttl time.Duration) error {
...
func (s *qdrantStore) Search(ctx context.Context, vec []float32, paramsHash string, limit int) ([]VecResult, error) {
	...
	for _, hit := range parsed.Result {
		...
		promptText, _ := hit.Payload["prompt_text"].(string)
