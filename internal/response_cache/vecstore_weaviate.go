...
func (s *weaviateStore) ensureClass(ctx context.Context) error {
	...
	schema := map[string]any{
		...
		"properties": []any{
			...
			map[string]any{"name": "prompt_text", "dataType": []string{"text"}},
		},
	}
...
func (s *weaviateStore) Insert(ctx context.Context, key string, vec []float32, response []byte, paramsHash string, promptText string, ttl time.Duration) error {
...
func (s *weaviateStore) runGraphQLSearch(ctx context.Context, query string) ([]VecResult, error) {
	...
	for _, it := range items {
		...
		out = append(out, VecResult{Key: it.CacheKey, Score: score, Response: rb, PromptText: it.PromptText})
	}
	return out, nil
}
