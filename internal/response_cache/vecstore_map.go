...
func (s *MapVecStore) Search(_ context.Context, vec []float32, paramsHash string, limit int) ([]VecResult, error) {
	...
	for i, e := range s.entries {
		...
		results = append(results, VecResult{Key: e.key, Score: score, Response: e.response, PromptText: e.promptText})
	}
	if len(toDelete) > 0 {
...
func (s *MapVecStore) Insert(_ context.Context, key string, vec []float32, response []byte, paramsHash string, promptText string, ttl time.Duration) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	var expiresAt int64
	if ttl != 0 {
