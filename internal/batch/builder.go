...
func New(ctx context.Context, cfg *config.Config) (*Result, error) {
	...
	backendCfg := cfg.Storage.BackendConfig()
	store, err := storage.New(ctx, backendCfg)
	if err != nil {
		return nil, fmt.Errorf("failed to create storage: %w", err)
	}
	batchStore, err := initStore(ctx, store)
	if err != nil {
		_ = store.Close()
		return nil, err
	}
	return &Result{Store: batchStore, Storage: store}, nil
}
...
func NewWithSharedStorage(ctx context.Context, shared storage.Storage) (*Result, error) {
	...
	batchStore, err := initStore(ctx, shared)
	if err != nil {
		return nil, err
	}
	return &Result{Store: batchStore}, nil
}

func initStore(ctx context.Context, store storage.Storage) (Store, error) {
