...
// Rerank delegates to the inner provider when it implements
// core.RerankRoutableProvider, returning a 501 GatewayError otherwise so the
// gateway can surface a clean unsupported_rerank_operation response.
func (p *Provider) Rerank(ctx context.Context, req *core.RerankRequest) (*core.RerankResponse, error) {
	rp, ok := p.inner.(core.RerankRoutableProvider)
	if !ok {
