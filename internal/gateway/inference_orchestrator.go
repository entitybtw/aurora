...
func NewInferenceOrchestrator(cfg InferenceConfig) *InferenceOrchestrator {
	return &InferenceOrchestrator{
		...
		promptCacheConfig:        cfg.PromptCacheConfig,
	}
}

// PromptCacheConfig returns the configured prompt cache config.
func (o *InferenceOrchestrator) PromptCacheConfig() *core.PromptCacheConfig {
	return o.promptCacheConfig
}

// ApplyPromptCache applies provider-level prompt caching to the request based on
// the provider type resolved in the workflow. This must be called after the
// provider is resolved but before the request is sent to the provider.
func (o *InferenceOrchestrator) ApplyPromptCache(req *core.ChatRequest, workflow *core.Workflow) {
	if req == nil || workflow == nil {
		return
	}
	providerType := ProviderTypeFromWorkflow(workflow)
	pc := promptcache.ResolvePromptCache(o.promptCacheConfig, req)
	if pc == nil || !pc.IsEnabled() {
		return
	}
	req.PromptCache = pc
	promptcache.ApplyPromptCache(req, pc, providerType)
}
...
// PreparedRerankRequest is a translated rerank request ready for execution.
type PreparedRerankRequest struct {
	Context  context.Context
	Request  *core.RerankRequest
	Workflow *core.Workflow
}
...
// RerankResult is the rerank result.
type RerankResult struct {
	Response *core.RerankResponse
	Meta     ExecutionMeta
}
