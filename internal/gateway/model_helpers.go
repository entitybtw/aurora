...
func CloneChatRequestForSelector(req *core.ChatRequest, selector core.ModelSelector) *core.ChatRequest {
	...
	cloned := *req
	cloned.Model = upstreamModelForSelector(req.Model, selector)
...
func upstreamModelForSelector(requestedModel string, selector core.ModelSelector) string {
...
func preserveRequestedModelForUpstream() bool {
	value := strings.ToLower(strings.TrimSpace(os.Getenv("AURORA_PRESERVE_REQUEST_MODEL_FOR_UPSTREAM")))
