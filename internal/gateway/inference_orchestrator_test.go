...
func TestTranslatedChatSelectorRewriteNotRequiredForResolvedProviderOnly(t *testing.T) {
	rewrite := translatedChatSelectorRewriteRequired(&core.RequestModelResolution{
		Requested:        core.NewRequestedModelSelector("gpt-4o-mini", ""),
		ResolvedSelector: core.ModelSelector{Provider: "openai-primary", Model: "gpt-4o-mini"},
	})
	if rewrite {
		t.Fatal("rewrite required for provider selected by routing but absent from request body")
	}
}
