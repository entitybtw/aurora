...
func ResolveRequestModelWithAuthorizer(
	...
	requested = applyRequestScopedRoutingPolicy(ctx, requested)
...
func applyRequestScopedRoutingPolicy(ctx context.Context, requested core.RequestedModelSelector) core.RequestedModelSelector {
	if keyID := strings.TrimSpace(core.GetProviderKeyID(ctx)); keyID != "" {
		return forceProviderHint(requested, keyID)
	}
	policy, ok := core.GetAuthKeyAccessPolicy(ctx)
	if !ok || policy.ProviderPoolID == nil {
		return requested
	}
	poolID := strings.TrimSpace(*policy.ProviderPoolID)
	if poolID == "" || requested.ExplicitProvider {
		return requested
	}
	return forceProviderHint(requested, poolID)
}

func forceProviderHint(requested core.RequestedModelSelector, providerHint string) core.RequestedModelSelector {
	...
	if err != nil {
		return core.NewRequestedModelSelector(requested.Model, providerHint)
	}
	if preserveRequestedModelForUpstream() {
		if requestedModel := strings.TrimSpace(requested.RequestedQualifiedModel()); requestedModel != "" {
			return core.NewRequestedModelSelector(requestedModel, providerHint)
		}
	}
	return core.NewRequestedModelSelector(selector.Model, providerHint)
}
