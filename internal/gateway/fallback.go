...
func executeTranslatedWithFallback[Req any, Resp any](
	...
	return executeWithFallbackResponse(ctx, o, workflow, model, provider,
		func() (Resp, string, string, error) {
			callReq := req
			if preserveRequestedModelForUpstream() && workflow != nil && workflow.Resolution != nil {
				callReq = cloneForSelector(req, workflow.Resolution.ResolvedSelector)
			}
			resp, responseProvider, err := call(ctx, callReq)
			if err != nil {
				var zero Resp
				return zero, "", "", err
			}
