...
func TestCreateListAndDeactivateAuthKey(t *testing.T) {
	...
	createReq := httptest.NewRequest(http.MethodPost, "/admin/api/v1/auth-keys", bytes.NewBufferString(`{"name":"primary","description":"prod key","user_path":" team//alpha/service/ ","rate_limits":{"requests_per_minute":12,"requests_per_day":1000,"tokens_per_minute":40000,"tokens_per_day":500000}}`))
	createReq.Header.Set("Content-Type", "application/json")
	createRec := httptest.NewRecorder()
	createCtx := e.NewContext(createReq, createRec)
	...
	wantLimits := authkeys.RateLimits{RequestsPerMinute: 12, RequestsPerDay: 1000, TokensPerMinute: 40000, TokensPerDay: 500000}
	if issued.RateLimits != wantLimits {
		t.Fatalf("issued.RateLimits = %#v, want %#v", issued.RateLimits, wantLimits)
	}
	...
	if views[0].RateLimits != wantLimits {
		t.Fatalf("views[0].RateLimits = %#v, want %#v", views[0].RateLimits, wantLimits)
	}
...
func TestCreateAuthKeyRejectsInvalidRateLimits(t *testing.T) {
	h := newAuthKeyHandler(t, newAuthKeyTestStore())
	e := echo.New()

	createReq := httptest.NewRequest(http.MethodPost, "/admin/api/v1/auth-keys", bytes.NewBufferString(`{"name":"primary","rate_limits":{"requests_per_minute":-1}}`))
	createReq.Header.Set("Content-Type", "application/json")
	createRec := httptest.NewRecorder()
	createCtx := e.NewContext(createReq, createRec)
