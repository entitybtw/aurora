		...
		versions: []workflow.Version{
			{
				...
				Payload: workflow.Payload{
					...
					Guardrails:    []workflow.GuardrailStep{{Ref: "policy-system", Step: 10}},
				},
				WorkflowHash: "hash-global",
			},
		},
	}
	planService, err := workflow.NewService(planStore, workflow.NewCompiler(guardrailService))
	if err != nil {
		t.Fatalf("workflow.NewService() error = %v", err)
	}
