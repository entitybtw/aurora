...
func (s *workflowTestStore) ListActive(context.Context) ([]workflow.Version, error) {
	result := make([]workflow.Version, 0, len(s.versions))
...
func (s *workflowTestStore) Get(_ context.Context, id string) (*workflow.Version, error) {
	for _, version := range s.versions {
		if version.ID == id {
			copy := version
			return &copy, nil
...
func (s *workflowTestStore) Create(_ context.Context, input workflow.CreateInput) (*workflow.Version, error) {
	var scopeKey string
	switch {
...
func (s *workflowTestStore) EnsureManagedDefaultGlobal(ctx context.Context, input workflow.CreateInput, workflowHash string) (*workflow.Version, error) {
	for _, version := range s.versions {
...
func workflowTestWorkflowHash(payload workflow.Payload) (string, error) {
...
func newWorkflowHandler(t *testing.T, store workflow.Store, registry *guardrails.Registry) *Handler {
	return newWorkflowHandlerWithModelRegistry(t, store, newWorkflowModelRegistry(t), registry)
}

func newWorkflowHandlerWithModelRegistry(t *testing.T, store workflow.Store, modelRegistry *providers.ModelRegistry, guardrailRegistry *guardrails.Registry) *Handler {
	t.Helper()

	service, err := workflow.NewService(store, workflow.NewCompiler(guardrailRegistry))
	if err != nil {
		...
		versions: []workflow.Version{
			{
				...
				Payload: workflow.Payload{
					SchemaVersion: 1,
					Features:      workflow.FeatureFlags{Cache: true, Audit: true, Usage: true, Guardrails: false, Fallback: &fallbackDisabled},
				},
	...
	store := &workflowTestStore{
		versions: []workflow.Version{
			...
			},
			{
				...
				Scope:       workflow.Scope{Provider: "openai", Model: "gpt-5"},
				...
				Payload: workflow.Payload{
					...
					Guardrails: []workflow.GuardrailStep{
		...
		versions: []workflow.Version{
			{
				...
				Payload: workflow.Payload{
					SchemaVersion: 1,
					Features:      workflow.FeatureFlags{Cache: true, Audit: true, Usage: true},
				},
				WorkflowHash: "hash-global",
			},
		},
		...
		versions: []workflow.Version{
			{
				...
				Payload: workflow.Payload{
					SchemaVersion: 1,
					Features:      workflow.FeatureFlags{Cache: true, Audit: true, Usage: true, Guardrails: false},
				},
				WorkflowHash: "hash-global",
			},
		},
...
func TestWorkflowViewReflectsFeatureCaps(t *testing.T) {
		...
		versions: []workflow.Version{
			{
				...
				Payload: workflow.Payload{
					SchemaVersion: 1,
					Features:      workflow.FeatureFlags{Cache: true, Audit: true, Usage: true, Guardrails: true},
				},
				WorkflowHash: "hash-global",
			},
		},
	...
	service, err := workflow.NewService(store, workflow.NewCompilerWithFeatureCaps(nil, core.WorkflowFeatures{
		Cache:      false,
		Audit:      true,
		Usage:      true,
		Guardrails: false,
	...
	store := &workflowTestStore{
		versions: []workflow.Version{
			...
			},
			{
				...
				Scope:    workflow.Scope{Provider: "openai"},
				...
				Payload: workflow.Payload{
					SchemaVersion: 1,
					Features:      workflow.FeatureFlags{Cache: false, Audit: true, Usage: true, Guardrails: false},
				},
