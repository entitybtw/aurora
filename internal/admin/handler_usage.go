...
// CacheDebug handles POST /admin/api/v1/cache/debug.
func (h *Handler) CacheDebug(c *echo.Context) error {
	if strings.TrimSpace(h.runtimeConfig.CacheEnabled) != "on" {
		return handleError(c, featureUnavailableError("cache debug is unavailable"))
	}
	if h.responseCache == nil {
		return handleError(c, featureUnavailableError("cache debug is unavailable"))
	}
	var req cacheDebugRequest
	if err := c.Bind(&req); err != nil {
		return handleError(c, core.NewInvalidRequestError("invalid request body: "+err.Error(), err))
	}
	...
	info, err := h.responseCache.DebugRequest(c.Request().Context(), method, path, req.Body, headers)
	if err != nil {
		return handleError(c, err)
	}
