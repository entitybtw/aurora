...
func TestModelRegistry(t *testing.T) {
	...
	t.Run("ConfiguredModelsAllowlistModeSkipsUpstreamAndUsesConfiguredModels", func(t *testing.T) {
		...
		if snapshots[0].LastModelFetchSuccessAt == nil {
			t.Fatal("LastModelFetchSuccessAt = nil, want non-nil: a clean allowlist resolution is a successful refresh — without this stamp the admin UI permanently reports allowlist-only providers as 'Starting'")
		}
