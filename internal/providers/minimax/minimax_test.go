...
func TestChatCompletion_MapsMiniMaxUsageLike9routerProviderUsage(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{
			"id":"chatcmpl-minimax-usage",
			"created":1677652288,
			"model":"MiniMax-M2.7",
			"choices":[{"index":0,"message":{"role":"assistant","content":"hello"},"finish_reason":"stop"}],
	...
	if resp.Usage.PromptTokens != 25 || resp.Usage.CompletionTokens != 7 || resp.Usage.TotalTokens != 32 {
		t.Fatalf("usage = %+v, want prompt=25 completion=7 total=32", resp.Usage)
	}
}
