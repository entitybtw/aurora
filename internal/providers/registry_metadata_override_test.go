...
func TestResolvePricingPrefersProviderSpecificMetadata(t *testing.T) {
	...
	if pricing == nil || pricing.Pricing.InputPerMtok == nil || *pricing.Pricing.InputPerMtok != backupRate {
...
func TestParseProvenanceTimestamp(t *testing.T) {
	tests := []struct {
		name   string
		input  string
		wantOK bool
	}{
		{name: "RFC3339 with Z", input: "2026-06-27T06:17:05Z", wantOK: true},
		{name: "space-separated no tz", input: "2026-06-27 06:17:05", wantOK: true},
		{name: "space-separated with offset", input: "2026-06-27 06:17:05 +00:00", wantOK: true},
		{name: "T-separated no tz", input: "2026-06-27T06:17:05", wantOK: true},
		{name: "empty string", input: "", wantOK: false},
		{name: "garbage", input: "not-a-timestamp", wantOK: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := parseProvenanceTimestamp(tt.input)
			...
			if tt.wantOK {
				...
				if got.Year() != 2026 || got.Month() != 6 || got.Day() != 27 {
					t.Errorf("expected 2026-06-27, got %v", got.UTC())
				}
			}
		})
	}
