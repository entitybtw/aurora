...
func NewPostgreSQLStore(ctx context.Context, pool *pgxpool.Pool) (*PostgreSQLStore, error) {
	...
	statements := []string{
		...
		`ALTER TABLE guardrail_definitions ADD COLUMN IF NOT EXISTS direction TEXT NOT NULL DEFAULT 'input'`,
...
func (s *PostgreSQLStore) Upsert(ctx context.Context, definition Definition) error {
	...
	`, definition.Name, definition.Type, definition.Direction, definition.Description, nullableString(definition.UserPath), definition.Config, definition.CreatedAt.Unix(), definition.UpdatedAt.Unix())
	if err != nil {
	...
	for _, definition := range definitions {
		...
		if _, err := tx.Exec(ctx, `
			...
			ON CONFLICT(name) DO UPDATE SET
				...
				config = excluded.config,
				updated_at = excluded.updated_at
		`, normalized.Name, normalized.Type, normalized.Direction, normalized.Description, nullableString(normalized.UserPath), normalized.Config, normalized.CreatedAt.Unix(), normalized.UpdatedAt.Unix()); err != nil {
