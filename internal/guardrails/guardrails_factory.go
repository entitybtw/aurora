...
func New(ctx context.Context, cfg *config.Config, refreshInterval time.Duration, executors ...ChatCompletionExecutor) (*Result, error) {
	...
	if !config.HasCapability(cfg.Edition, "advancedGuardrails") {
		DisableAdvancedGuardrails()
	}
...
func NewWithSharedStorage(ctx context.Context, shared storage.Storage, cfg *config.Config, refreshInterval time.Duration, executors ...ChatCompletionExecutor) (*Result, error) {
	if shared == nil {
	...
	if cfg != nil && !config.HasCapability(cfg.Edition, "advancedGuardrails") {
		DisableAdvancedGuardrails()
	}
