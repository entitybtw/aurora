...
type ContextPipelineResolver interface {
	...
	OutputPipelineForContext(ctx context.Context) *Pipeline
}
...
// PatchChatResponse applies the request-scoped output guardrails pipeline to a chat response.
func (p *WorkflowRequestPatcher) PatchChatResponse(ctx context.Context, resp *core.ChatResponse) (*core.ChatResponse, error) {
	return processGuardedChatResponse(ctx, p.outputPipeline(ctx), resp)
}

// PatchResponsesResponse applies the request-scoped output guardrails pipeline to a Responses response.
func (p *WorkflowRequestPatcher) PatchResponsesResponse(ctx context.Context, resp *core.ResponsesResponse) (*core.ResponsesResponse, error) {
	return processGuardedResponsesResponse(ctx, p.outputPipeline(ctx), resp)
}
...
func (p *WorkflowRequestPatcher) outputPipeline(ctx context.Context) *Pipeline {
	if p == nil || p.resolver == nil {
		return nil
	}
	return p.resolver.OutputPipelineForContext(ctx)
}
