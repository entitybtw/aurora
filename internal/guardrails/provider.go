...
func NewGuardedProviderWithOptions(inner core.RoutableProvider, pipeline *Pipeline, options Options) *GuardedProvider {
	var outputPipeline *Pipeline
	if options.EnableOutputGuardrails {
		outputPipeline = pipeline
	}
	return NewGuardedProviderWithPipelines(inner, pipeline, outputPipeline, options)
}

// NewGuardedProviderWithPipelines creates a RoutableProvider with separate input and output pipelines.
func NewGuardedProviderWithPipelines(inner core.RoutableProvider, inputPipeline, outputPipeline *Pipeline, options Options) *GuardedProvider {
...
func (g *GuardedProvider) ChatCompletion(ctx context.Context, req *core.ChatRequest) (*core.ChatResponse, error) {
	...
	modified, err := processGuardedChat(ctx, g.inputPipeline, req)
	if err != nil {
		return nil, err
	}
	resp, err := g.inner.ChatCompletion(ctx, modified)
	if err != nil {
		return nil, err
	}
	return processGuardedChatResponse(ctx, g.outputPipeline, resp)
}
...
// Rerank delegates directly to the inner provider when it implements
// core.RerankRoutableProvider. No guardrails are applied to rerank queries.
func (g *GuardedProvider) Rerank(ctx context.Context, req *core.RerankRequest) (*core.RerankResponse, error) {
	rp, ok := g.inner.(core.RerankRoutableProvider)
	if !ok {
		return nil, core.NewInvalidRequestError("rerank routing is not supported by the current provider router", nil)
	}
	return rp.Rerank(ctx, req)
}
...
func (g *GuardedProvider) Responses(ctx context.Context, req *core.ResponsesRequest) (*core.ResponsesResponse, error) {
	...
	modified, err := processGuardedResponses(ctx, g.inputPipeline, req)
	if err != nil {
		return nil, err
	}
	resp, err := g.inner.Responses(ctx, modified)
	if err != nil {
		return nil, err
	}
	return processGuardedResponsesResponse(ctx, g.outputPipeline, resp)
}
...
func (g *GuardedProvider) CreateBatch(ctx context.Context, providerType string, req *core.BatchRequest) (*core.BatchResponse, error) {
	...
	result, err := processGuardedBatchRequest(ctx, providerType, req, g.inputPipeline, g.batchFileTransport())
	if err != nil {
		return nil, err
	}
...
func (g *GuardedProvider) PatchChatRequest(ctx context.Context, req *core.ChatRequest) (*core.ChatRequest, error) {
	return processGuardedChat(ctx, g.inputPipeline, req)
}
...
func (g *GuardedProvider) PatchResponsesRequest(ctx context.Context, req *core.ResponsesRequest) (*core.ResponsesRequest, error) {
	return processGuardedResponses(ctx, g.inputPipeline, req)
}
...
func (g *GuardedProvider) PrepareBatchRequest(ctx context.Context, providerType string, req *core.BatchRequest) (*core.BatchRewriteResult, error) {
	...
	return processGuardedBatchRequest(ctx, providerType, req, g.inputPipeline, g.batchFileTransport())
}
