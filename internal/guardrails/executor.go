...
// PatchChatResponse applies guardrails to a translated chat response.
func (p *RequestPatcher) PatchChatResponse(ctx context.Context, resp *core.ChatResponse) (*core.ChatResponse, error) {
	return processGuardedChatResponse(ctx, nil, resp)
}

// PatchResponsesResponse applies guardrails to a translated Responses response.
func (p *RequestPatcher) PatchResponsesResponse(ctx context.Context, resp *core.ResponsesResponse) (*core.ResponsesResponse, error) {
	return processGuardedResponsesResponse(ctx, nil, resp)
}
...
func processGuardedChat(ctx context.Context, pipeline *Pipeline, req *core.ChatRequest) (*core.ChatRequest, error) {
	...
	if err != nil {
		return nil, markGuardrailError(err)
	}
...
func processGuardedChatResponse(ctx context.Context, pipeline *Pipeline, resp *core.ChatResponse) (*core.ChatResponse, error) {
	if pipeline == nil || pipeline.Len() == 0 || resp == nil {
		return resp, nil
	}
	msgs := chatResponseToMessages(resp)
	modified, err := pipeline.Process(ctx, msgs)
	if err != nil {
		return nil, markGuardrailError(err)
	}
	return applyMessagesToChatResponse(resp, modified)
}

func processGuardedResponsesResponse(ctx context.Context, pipeline *Pipeline, resp *core.ResponsesResponse) (*core.ResponsesResponse, error) {
	if pipeline == nil || pipeline.Len() == 0 || resp == nil {
		return resp, nil
	}
	msgs := responsesResponseToMessages(resp)
	modified, err := pipeline.Process(ctx, msgs)
	if err != nil {
		return nil, markGuardrailError(err)
	}
	return applyMessagesToResponsesResponse(resp, modified)
}
