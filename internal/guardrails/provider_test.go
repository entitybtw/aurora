...
func (m *mockRoutableProvider) Responses(_ context.Context, req *core.ResponsesRequest) (*core.ResponsesResponse, error) {
	m.responsesReq = req
	if m.responsesResp != nil {
		return m.responsesResp, nil
	}
...
func TestGuardedProvider_ChatCompletion_AppliesOutputGuardrails(t *testing.T) {
	inner := &mockRoutableProvider{
		chatResp: &core.ChatResponse{
			Model: "gpt-4",
			Choices: []core.Choice{{
				Message: core.ResponseMessage{Role: "assistant", Content: "secret value"},
			}},
		},
	}
	pipeline := NewPipeline()
	gr, err := NewRegexBlockGuardrail("output-mask", RegexBlockConfig{Action: RegexBlockActionSanitize, Patterns: []string{"secret"}, Replacement: "[MASKED]"})
	if err != nil {
		t.Fatalf("NewRegexBlockGuardrail() error = %v", err)
	}
	pipeline.Add(gr, 0)

	guarded := NewGuardedProviderWithOptions(inner, pipeline, Options{EnableOutputGuardrails: true})

	resp, err := guarded.ChatCompletion(context.Background(), &core.ChatRequest{Model: "gpt-4", Messages: []core.Message{{Role: "user", Content: "hi"}}})
	if err != nil {
		t.Fatalf("ChatCompletion() error = %v", err)
	}
	if resp == nil || len(resp.Choices) != 1 {
		t.Fatalf("ChatCompletion() response = %#v", resp)
	}
	if got := core.ExtractTextContent(resp.Choices[0].Message.Content); got != "[MASKED] value" {
		t.Fatalf("output content = %q, want sanitized response", got)
	}
	if got := core.ExtractTextContent(inner.chatResp.Choices[0].Message.Content); got != "secret value" {
		t.Fatalf("provider response was mutated: %q", got)
	}
}

func TestGuardedProvider_Responses_AppliesOutputGuardrails(t *testing.T) {
	inner := &mockRoutableProvider{
		responsesResp: &core.ResponsesResponse{
			Model: "gpt-4",
			Output: []core.ResponsesOutputItem{{
				...
				Type:    "message",
				Role:    "assistant",
				Content: []core.ResponsesContentItem{{Type: "output_text", Text: "secret value"}},
			}},
		},
	}
	pipeline := NewPipeline()
	...
	resp, err := guarded.Responses(context.Background(), &core.ResponsesRequest{Model: "gpt-4", Input: "hi"})
	if err != nil {
	...
	if got := resp.Output[0].Content[0].Text; got != "[MASKED] value" {
		t.Fatalf("output text = %q, want sanitized response", got)
	}
	if got := inner.responsesResp.Output[0].Content[0].Text; got != "secret value" {
		t.Fatalf("provider response was mutated: %q", got)
	}
}

func TestGuardedProvider_ChatCompletion_BlocksUnsafeOutput(t *testing.T) {
	inner := &mockRoutableProvider{
		chatResp: &core.ChatResponse{
			Model: "gpt-4",
			Choices: []core.Choice{{
	...
	pipeline := NewPipeline()
	gr, err := NewRegexBlockGuardrail("output-block", RegexBlockConfig{Action: RegexBlockActionBlock, Patterns: []string{"secret"}})
	if err != nil {
		t.Fatalf("NewRegexBlockGuardrail() error = %v", err)
	}
	pipeline.Add(gr, 0)
	...
	_, err = guarded.ChatCompletion(context.Background(), &core.ChatRequest{Model: "gpt-4", Messages: []core.Message{{Role: "user", Content: "hi"}}})
	if err == nil {
		t.Fatal("ChatCompletion() error = nil, want guardrail block")
	}
	var gatewayErr *core.GatewayError
	if !errors.As(err, &gatewayErr) {
		t.Fatalf("ChatCompletion() error = %T, want *core.GatewayError", err)
	}
	if gatewayErr.Code == nil || *gatewayErr.Code != "guardrail_blocked" {
		t.Fatalf("gateway error code = %#v, want guardrail_blocked", gatewayErr.Code)
	}
}
