...
func (s *Service) BuildPipeline(steps []StepReference) (*Pipeline, string, error) {
	inputPipeline, _, hash, err := s.BuildPipelines(steps)
	return inputPipeline, hash, err
}

// BuildPipelines resolves named steps into input and output guardrail pipelines.
func (s *Service) BuildPipelines(steps []StepReference) (*Pipeline, *Pipeline, string, error) {
	if len(steps) == 0 {
		return nil, nil, "", nil
	}
	...
	s.mu.RUnlock()
	if registry == nil {
		return nil, nil, "", core.NewProviderError("", http.StatusBadGateway, "guardrail catalog is not loaded", nil)
	}
	return registry.BuildPipelines(steps)
}
...
func buildSnapshot(definitions []Definition, executor ChatCompletionExecutor) (serviceSnapshot, error) {
	...
	for _, definition := range definitions {
		...
		if err := next.registry.RegisterWithDirection(instance, normalized.Direction, descriptor); err != nil {
