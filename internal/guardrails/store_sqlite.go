...
func NewSQLiteStore(ctx context.Context, db *sql.DB) (*SQLiteStore, error) {
	...
	statements := []string{
		...
		`ALTER TABLE guardrail_definitions ADD COLUMN direction TEXT NOT NULL DEFAULT 'input'`,
...
func (s *SQLiteStore) List(ctx context.Context) ([]Definition, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT name, type, direction, description, user_path, config, created_at, updated_at
...
func (s *SQLiteStore) Upsert(ctx context.Context, definition Definition) error {
	...
	_, err = s.db.ExecContext(ctx, `
		INSERT INTO guardrail_definitions (name, type, direction, description, user_path, config, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	...
	`, definition.Name, definition.Type, definition.Direction, definition.Description, nullableString(definition.UserPath), string(definition.Config), definition.CreatedAt.Unix(), definition.UpdatedAt.Unix())
	if err != nil {
	...
	for _, definition := range definitions {
		...
		if _, err := tx.ExecContext(ctx, `
			...
			ON CONFLICT(name) DO UPDATE SET
				...
				config = excluded.config,
				updated_at = excluded.updated_at
		`, normalized.Name, normalized.Type, normalized.Direction, normalized.Description, nullableString(normalized.UserPath), string(normalized.Config), normalized.CreatedAt.Unix(), normalized.UpdatedAt.Unix()); err != nil {
...
func scanSQLiteDefinition(scanner definitionScanner) (Definition, error) {
	...
	definition.Direction = normalizeDirection(nullableStringValue(direction))
