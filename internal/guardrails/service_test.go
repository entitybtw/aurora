...
func TestServiceRefreshBuildsDirectionAwarePipelines(t *testing.T) {
	store := newTestStore(
		Definition{
			Name:      "input-policy",
			Type:      "system_prompt",
	...
	inputPipeline, outputPipeline, hash, err := service.BuildPipelines([]StepReference{{Ref: "input-policy", Step: 0}, {Ref: "output-mask", Step: 10}})
	if err != nil {
		t.Fatalf("BuildPipelines() error = %v", err)
	}
	if inputPipeline == nil || inputPipeline.Len() != 1 {
		t.Fatalf("inputPipeline = %#v, want one input guardrail", inputPipeline)
	}
	if outputPipeline == nil || outputPipeline.Len() != 1 {
		t.Fatalf("outputPipeline = %#v, want one output guardrail", outputPipeline)
	}
	if hash == "" {
		t.Fatal("BuildPipelines() hash = empty, want non-empty")
	}

	inputMsgs, err := inputPipeline.Process(context.Background(), []Message{{Role: "user", Content: "hi"}})
	if err != nil {
		t.Fatalf("inputPipeline.Process() error = %v", err)
	}
	if len(inputMsgs) != 2 || inputMsgs[0].Content != "be safe" {
		t.Fatalf("inputPipeline messages = %#v, want injected system prompt", inputMsgs)
	}

	outputMsgs, err := outputPipeline.Process(context.Background(), []Message{{Role: "assistant", Content: "secret value"}})
	if err != nil {
		t.Fatalf("outputPipeline.Process() error = %v", err)
	}
	if outputMsgs[0].Content != "[MASKED] value" {
		t.Fatalf("outputPipeline messages = %#v, want sanitized output", outputMsgs)
	}
}
