...
func TestServerConfig_DisablesPassthroughSemanticEnrichmentMiddleware(t *testing.T) {
	provider := &mockProvider{}
	server := New(provider, &Config{
		PassthroughSemanticEnrichers:         []core.PassthroughSemanticEnricher{passthroughSemanticEnricherStub{providerType: "openai"}},
		DisablePassthroughSemanticEnrichment: true,
	})
	...
	if rec.Code == http.StatusOK && strings.Contains(rec.Body.String(), "openai.responses") {
		t.Fatal("passthrough semantic enrichment appears to be enabled")
	}
}
