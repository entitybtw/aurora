...
type mockAuthenticator struct {
	...
	tokenLimits map[string]authkeys.RateLimits
	err         error
}
...
func (m mockAuthenticator) Authenticate(_ context.Context, token string) (authkeys.AuthenticationResult, error) {
	...
	return authkeys.AuthenticationResult{
		...
		TenantID:   m.tokenTenant[token],
		RateLimits: m.tokenLimits[token],
	}, nil
}
...
func TestAuthMiddleware(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "missing authorization header - denies request when auth configured",
			masterKey:      "secret-key-123",
			authHeader:     "",
		...
		},
		{
			...
			expectedStatus: http.StatusUnauthorized,
			expectedBody:   `{"error":{"message":"invalid master key or session","type":"authentication_error","param":null,"code":null}}`,
		},
		{
		...
		{
			name:           "empty bearer token - denies request when auth configured",
			masterKey:      "secret-key-123",
			authHeader:     "Bearer ",
			expectedStatus: http.StatusUnauthorized,
			expectedBody:   `{"error":{"message":"missing authorization header","type":"authentication_error","param":null,"code":null}}`,
...
func TestAuthMiddleware_Integration(t *testing.T) {
	t.Run("with master key - protects all routes", func(t *testing.T) {
		...
		// Request without auth is denied when auth is configured.
...
func TestAuthMiddlewareWithAuthenticator_ManagedKeyTenantEnrichesContext(t *testing.T) {
	e := echo.New()
	testHandler := func(c *echo.Context) error {
		if got := scope.EffectiveID(c.Request().Context()); got != "tenant-a" {
	...
	handler := AuthMiddlewareWithAuthenticator("", mockAuthenticator{
		enabled:     true,
		tokenToID:   map[string]string{"sk_gom_token": "key-123"},
		tokenTenant: map[string]string{"sk_gom_token": "tenant-a"},
	}, nil)(testHandler)
...
func TestAuthMiddlewareWithAuthenticator_ManagedKeyRateLimitsEnrichContext(t *testing.T) {
	e := echo.New()
	limits := authkeys.RateLimits{RequestsPerMinute: 3, RequestsPerDay: 20}
	testHandler := func(c *echo.Context) error {
		got, ok := core.GetAuthKeyRateLimits(c.Request().Context())
		if !ok {
			t.Fatal("auth key rate limits missing from context")
		}
		if got != limits {
			t.Fatalf("auth key rate limits = %#v, want %#v", got, limits)
		}
		return c.String(http.StatusOK, "ok")
	}
	...
	handler := AuthMiddlewareWithAuthenticator("", mockAuthenticator{
		enabled:     true,
		tokenToID:   map[string]string{"sk_gom_token": "key-123"},
		tokenLimits: map[string]authkeys.RateLimits{"sk_gom_token": limits},
	}, nil)(testHandler)
...
func TestAuthMiddleware_SkipPaths(t *testing.T) {
	t.Run("skips authentication for specified paths", func(t *testing.T) {
		...
		// Request to protected path without auth is denied when auth is configured.
	...
	t.Run("empty skip paths requires auth for all routes", func(t *testing.T) {
		...
		// Request without auth is denied when auth is configured and no skip paths are set.
		req := httptest.NewRequest(http.MethodGet, "/health", nil)
		rec := httptest.NewRecorder()
		e.ServeHTTP(rec, req)
		assert.Equal(t, http.StatusUnauthorized, rec.Code)
