...
func (r *readTrackingReadCloser) Read(p []byte) (int, error) {
	r.reads++
	return r.Reader.Read(p)
}

func (r *readTrackingReadCloser) Close() error {
	return nil
}
...
func TestRequestSnapshotCaptureWithOptions_DisableBodySnapshotKeepsRequestIDWithoutReadingBody(t *testing.T) {
	e := echo.New()
	body := &readTrackingReadCloser{Reader: strings.NewReader(`{"model":"gpt-4o-mini","messages":[]}`)}
	...
	var capturedFrame *core.RequestSnapshot
	var capturedEnv *core.WhiteBoxPrompt
	handler := RequestSnapshotCaptureWithOptions(RequestSnapshotOptions{DisableBodySnapshot: true})(func(c *echo.Context) error {
		capturedFrame = core.GetRequestSnapshot(c.Request().Context())
		capturedEnv = core.GetWhiteBoxPrompt(c.Request().Context())
	...
	if capturedFrame != nil {
		t.Fatalf("snapshot = %v, want nil", capturedFrame)
	}
	if capturedEnv != nil {
		t.Fatalf("white-box prompt = %v, want nil", capturedEnv)
	}
