...
func TestMetricsEndpointCustomPaths(t *testing.T) {
	...
	t.Run("custom metrics path requires auth", func(t *testing.T) {
		srv := New(mock, &Config{
		...
		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated custom metrics path, got %d", rec.Code)
		}
		...
		if rec.Code != http.StatusOK {
			t.Errorf("Expected 200 for authenticated custom metrics path, got %d", rec.Code)
		}
	})
	...
	t.Run("nested metrics path works", func(t *testing.T) {
		...
		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated nested metrics path, got %d", rec.Code)
		}
	})
}
...
func TestMetricsEndpointAPIRouteProtection(t *testing.T) {
	...
	t.Run("metrics at /v1/metrics falls back to /metrics", func(t *testing.T) {
		...
		// /v1/metrics should 404 (not registered as metrics endpoint, not an API route)
		req := httptest.NewRequest(http.MethodGet, "/v1/metrics", nil)
		rec := httptest.NewRecorder()
		srv.ServeHTTP(rec, req)

		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated /v1/metrics, got %d", rec.Code)
		}
		...
		if rec2.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated /metrics fallback, got %d", rec2.Code)
		}
	})
	...
	t.Run("metrics at /v1/models falls back to /metrics", func(t *testing.T) {
		...
		// /v1/models should remain protected by auth.
		req := httptest.NewRequest(http.MethodGet, "/v1/models", nil)
		rec := httptest.NewRecorder()
		srv.ServeHTTP(rec, req)

		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated /v1/models, got %d", rec.Code)
		}
	})
	...
	t.Run("path traversal to /v1/ is blocked", func(t *testing.T) {
		...
		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated /metrics fallback, got %d", rec.Code)
		}
	})
}
...
func TestMetricsEndpointPathTraversal(t *testing.T) {
	...
	t.Run("path traversal is normalized", func(t *testing.T) {
		...
		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated normalized path /admin, got %d", rec.Code)
		}
	})
	...
	t.Run("double dots are cleaned from path", func(t *testing.T) {
		...
		if rec.Code != http.StatusUnauthorized {
			t.Errorf("Expected 401 for unauthenticated cleaned path /a/c, got %d", rec.Code)
		}
	})
}
...
func TestPprofEndpoint_RequiresAuthWhenDisabled(t *testing.T) {
	...
	if rec.Code != http.StatusUnauthorized {
		t.Errorf("expected 401 before pprof disabled routing, got %d", rec.Code)
	}
