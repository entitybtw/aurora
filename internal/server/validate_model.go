...
func WorkflowResolutionWithResolverAndPolicy(
	...
	benchFastPath := minimalBenchModeEnabled() && nonStreamingChatPassthroughFastPathEnabled()
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c *echo.Context) error {
			...
			if benchFastPath {
				desc := core.DescribeEndpoint(c.Request().Method, path)
				if desc.Operation == core.OperationChatCompletions {
					...
					if env := core.GetWhiteBoxPrompt(c.Request().Context()); env != nil && env.RouteHints.Model != "" {
						workflow.Resolution = &core.RequestModelResolution{
							Requested: core.NewRequestedModelSelector(env.RouteHints.Model, ""),
					...
					if body, err := requestBodyBytes(c); err == nil {
						if model, _, parsed := selectorHintsFromJSONGJSON(body); parsed && model != "" {
							workflow.Resolution = &core.RequestModelResolution{
