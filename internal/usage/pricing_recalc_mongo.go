...
func mongoRecalculationUpdate(update recalculationUpdate) bson.D {
	...
	// Pricing provenance
	if strings.TrimSpace(update.PricingProvenanceSource) != "" {
		set = append(set, bson.E{Key: "pricing_source", Value: strings.TrimSpace(update.PricingProvenanceSource)})
		_ = unset // keep compiler happy if len is 0
	} else {
		unset = append(unset, bson.E{Key: "pricing_source", Value: ""})
	}
	if strings.TrimSpace(update.PricingProvenanceVersion) != "" {
		set = append(set, bson.E{Key: "pricing_version", Value: strings.TrimSpace(update.PricingProvenanceVersion)})
	} else {
		unset = append(unset, bson.E{Key: "pricing_version", Value: ""})
	}
	if update.PricingProvenanceResolvedAt != nil && !update.PricingProvenanceResolvedAt.IsZero() {
		set = append(set, bson.E{Key: "pricing_resolved_at", Value: *update.PricingProvenanceResolvedAt})
	} else {
		unset = append(unset, bson.E{Key: "pricing_resolved_at", Value: ""})
	}
