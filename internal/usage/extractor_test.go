...
func TestExtractFromChatResponse_WithPricing(t *testing.T) {
	pricing := &PricingResult{Pricing: &core.ModelPricing{
	...
	if entry.PricingSource != "" {
		t.Errorf("PricingSource = %q, want empty (no provenance set)", entry.PricingSource)
	}
	if entry.PricingVersion != "" {
		t.Errorf("PricingVersion = %q, want empty (no provenance set)", entry.PricingVersion)
	}
	if entry.PricingResolvedAt != nil {
		t.Errorf("PricingResolvedAt = %v, want nil (no provenance set)", entry.PricingResolvedAt)
	}
}
...
func TestExtractFromChatResponse_OpenRouterCreditCostWithoutStaticPricing(t *testing.T) {
	...
	if entry.PricingSource != "provider_reported" {
		t.Fatalf("PricingSource = %q, want %q", entry.PricingSource, "provider_reported")
	}
}
...
func TestApplyUsageCosts_SetsProvenanceFromPricingResult(t *testing.T) {
	...
	applyUsageCosts(entry, "openai", "/v1/chat/completions", pricingResult)
	...
	if entry.PricingSource != "upstream_registry" {
		t.Fatalf("PricingSource = %q, want %q", entry.PricingSource, "upstream_registry")
	}
	if entry.PricingVersion != "42" {
		t.Fatalf("PricingVersion = %q, want %q", entry.PricingVersion, "42")
	}
	if entry.PricingResolvedAt == nil || !entry.PricingResolvedAt.Equal(now) {
		t.Fatalf("PricingResolvedAt = %v, want %v", entry.PricingResolvedAt, now)
	}
}

func TestApplyUsageCosts_ProviderReportedSetsProviderReportedSource(t *testing.T) {
	t.Run("openrouter", func(t *testing.T) {
		...
		applyUsageCosts(entry, "openrouter", "/v1/chat/completions")
		if entry.CostSource != CostSourceOpenRouterCredits {
			t.Fatalf("CostSource = %q, want %q", entry.CostSource, CostSourceOpenRouterCredits)
		}
		if entry.PricingSource != "provider_reported" {
		...
		if entry.PricingVersion != "" {
			t.Fatalf("PricingVersion = %q, want empty", entry.PricingVersion)
		}
		if entry.PricingResolvedAt != nil {
			t.Fatalf("PricingResolvedAt = %v, want nil", entry.PricingResolvedAt)
		}
	})
	...
	t.Run("xai", func(t *testing.T) {
		...
		applyUsageCosts(entry, "xai", "/v1/chat/completions")
		if entry.CostSource != CostSourceXAITicks {
			t.Fatalf("CostSource = %q, want %q", entry.CostSource, CostSourceXAITicks)
		}
		if entry.PricingSource != "provider_reported" {
	...
	t.Run("with pricing result ignored for provider_reported", func(t *testing.T) {
		entry := &UsageEntry{
			InputTokens:  100,
			OutputTokens: 50,
			TotalTokens:  150,
			RawData:      map[string]any{"cost": 0.00050},
		}
		now := time.Date(2026, 7, 14, 12, 0, 0, 0, time.UTC)
		pricingResult := &PricingResult{
			Pricing: &core.ModelPricing{
		...
		applyUsageCosts(entry, "openrouter", "/v1/chat/completions", pricingResult)
		if entry.CostSource != CostSourceOpenRouterCredits {
			t.Fatalf("CostSource = %q, want %q", entry.CostSource, CostSourceOpenRouterCredits)
		}
		if entry.PricingSource != "provider_reported" {
		...
		// Provenance from PricingResult must NOT leak through for provider-reported costs
		if entry.PricingVersion != "" {
			t.Fatalf("PricingVersion = %q, want empty", entry.PricingVersion)
		}
		if entry.PricingResolvedAt != nil {
