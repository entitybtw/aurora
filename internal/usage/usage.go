...
type UsageEntry struct {
	...
	// Pricing provenance — which pricing data was used to compute the cost estimate.
	// These fields enable traceability when provider prices change or admin overrides
	// are adjusted, so old reports remain explainable after pricing updates.
	PricingSource    string     `json:"pricing_source,omitempty" bson:"pricing_source,omitempty"`
	PricingVersion   string     `json:"pricing_version,omitempty" bson:"pricing_version,omitempty"`
	PricingResolvedAt *time.Time `json:"pricing_resolved_at,omitempty" bson:"pricing_resolved_at,omitempty"`
}
