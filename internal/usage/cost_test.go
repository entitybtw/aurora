...
func TestCalculateGranularCost_IncludedInputDetailExceedingBaseDoesNotGoNegative(t *testing.T) {
	pricing := &core.ModelPricing{
		InputPerMtok:       new(0.30),
	...
	result := CalculateGranularCost(1, 0, rawData, "gemini", pricing)

	assertCostNear(t, "InputCost", result.InputCost, 0.0000003)
	assertCostNear(t, "TotalCost", result.TotalCost, 0.0000003)
	if *result.InputCost < 0 || *result.TotalCost < 0 {
		t.Fatalf("costs must not be negative: input=%v total=%v", *result.InputCost, *result.TotalCost)
	}
	if result.Caveat != "prompt_cached_tokens exceeds remaining base input tokens; overflow priced at mapped rate" {
		t.Fatalf("unexpected caveat: %q", result.Caveat)
	}
}

func TestCalculateGranularCost_MultipleIncludedInputDetailsCannotOverDiscountBase(t *testing.T) {
	pricing := &core.ModelPricing{
		InputPerMtok:       new(0.30),
		CachedInputPerMtok: new(0.075),
		AudioInputPerMtok:  new(0.06),
	}
	rawData := map[string]any{
		"prompt_cached_tokens": 1,
		"prompt_audio_tokens":  4,
	}
	result := CalculateGranularCost(1, 0, rawData, "openai", pricing)

	assertCostNear(t, "InputCost", result.InputCost, 0.000000315)
	assertCostNear(t, "TotalCost", result.TotalCost, 0.000000315)
	if *result.InputCost < 0 || *result.TotalCost < 0 {
		t.Fatalf("costs must not be negative: input=%v total=%v", *result.InputCost, *result.TotalCost)
	}
	if result.Caveat != "prompt_audio_tokens exceeds remaining base input tokens; overflow priced at mapped rate" {
		t.Fatalf("unexpected caveat: %q", result.Caveat)
	}
}

func TestCalculateGranularCost_IncludedOutputDetailExceedingBaseDoesNotGoNegative(t *testing.T) {
	pricing := &core.ModelPricing{
		OutputPerMtok:          new(10.0),
		ReasoningOutputPerMtok: new(2.0),
	}
	rawData := map[string]any{
		"completion_reasoning_tokens": 4,
	}
	result := CalculateGranularCost(0, 1, rawData, "openai", pricing)

	assertCostNear(t, "OutputCost", result.OutputCost, 0.000008)
	assertCostNear(t, "TotalCost", result.TotalCost, 0.000008)
	if *result.OutputCost < 0 || *result.TotalCost < 0 {
		t.Fatalf("costs must not be negative: output=%v total=%v", *result.OutputCost, *result.TotalCost)
	}
	if result.Caveat != "completion_reasoning_tokens exceeds remaining base output tokens; overflow priced at mapped rate" {
		t.Fatalf("unexpected caveat: %q", result.Caveat)
	}
}

func TestCalculateGranularCost_NegativeTokenCountsAreIgnored(t *testing.T) {
	pricing := &core.ModelPricing{
		InputPerMtok:       new(3.0),
		OutputPerMtok:      new(10.0),
	...
	result := CalculateGranularCost(-1, -2, rawData, "openai", pricing)

	assertCostNear(t, "InputCost", result.InputCost, 0)
	assertCostNear(t, "OutputCost", result.OutputCost, 0)
	assertCostNear(t, "TotalCost", result.TotalCost, 0)
	want := "negative input tokens ignored; negative output tokens ignored; negative token field ignored: cached_tokens"
	if result.Caveat != want {
		t.Fatalf("Caveat = %q, want %q", result.Caveat, want)
	}
}

func TestCalculateGranularCost_NonFiniteFloatTokenCountIgnored(t *testing.T) {
	pricing := &core.ModelPricing{
		InputPerMtok:       new(3.0),
		CachedInputPerMtok: new(0.3),
	}
	rawData := map[string]any{
		"cached_tokens": math.Inf(1),
	}
	result := CalculateGranularCost(1, 0, rawData, "openai", pricing)

	assertCostNear(t, "InputCost", result.InputCost, 0.000003)
	assertCostNear(t, "TotalCost", result.TotalCost, 0.000003)
	if result.Caveat != "" {
		t.Fatalf("expected empty caveat, got %q", result.Caveat)
	}
}
