...
var usagePartialWriteFailures = promauto.NewCounter(
	prometheus.CounterOpts{
		Name: "aurora_usage_partial_write_failures_total",
...
func NewMongoDBStore(database *mongo.Database, retentionDays int) (*MongoDBStore, error) {
	...
	indexes := []mongo.IndexModel{
		...
		{
			Keys: bson.D{{Key: "tenant_id", Value: 1}, {Key: "user_path", Value: 1}},
		},
		{
	...
	if err := migrateMongoLegacyTenantUsagePaths(ctx, collection); err != nil {
		return nil, err
	}
...
func migrateMongoLegacyTenantUsagePaths(ctx context.Context, collection *mongo.Collection) error {
	cursor, err := collection.Find(ctx, bson.D{{Key: "user_path", Value: bson.D{{Key: "$regex", Value: "^/_tenants/"}}}})
	if err != nil {
		return fmt.Errorf("find legacy tenant usage paths: %w", err)
	}
	...
	for cursor.Next(ctx) {
		...
		if err := cursor.Decode(&doc); err != nil {
			return fmt.Errorf("decode legacy tenant usage path: %w", err)
		}
		legacyTenantID, userPath, ok := splitLegacyTenantUserPath(doc.UserPath)
		if !ok {
			continue
		}
		...
		_, err := collection.UpdateOne(ctx,
			...
			bson.D{{Key: "$set", Value: bson.D{{Key: "tenant_id", Value: tenantID}, {Key: "user_path", Value: userPath}}}},
		)
		if err != nil {
			return fmt.Errorf("migrate legacy tenant usage path: %w", err)
		}
	}
	if err := cursor.Err(); err != nil {
		return fmt.Errorf("iterate legacy tenant usage paths: %w", err)
	}
	return nil
}
