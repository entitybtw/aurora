...
func NewSQLiteStore(db *sql.DB, retentionDays int) (*SQLiteStore, error) {
	...
	costMigrations := []string{
		...
		"ALTER TABLE usage ADD COLUMN pricing_source TEXT DEFAULT ''",
		"ALTER TABLE usage ADD COLUMN pricing_version TEXT DEFAULT ''",
		"ALTER TABLE usage ADD COLUMN pricing_resolved_at DATETIME",
	}
	...
	for i := 0; i < len(entries); i += maxEntriesPerBatch {
		...
		for j, e := range chunk {
			...
			values = append(values,
				...
				e.PricingSource,
				e.PricingVersion,
				pricingResolvedAtValue(e.PricingResolvedAt),
			)
		}
		...
		query := `INSERT OR IGNORE INTO usage (id, request_id, provider_id, timestamp, model, provider, provider_name,
			...
			pricing_source, pricing_version, pricing_resolved_at) VALUES ` +
...
// pricingResolvedAtValue converts a *time.Time to a string for SQLite storage.
func pricingResolvedAtValue(t *time.Time) any {
	if t == nil || t.IsZero() {
		return nil
	}
