...
func clampConversationLimit(limit int) int {
	// Hard cap the conversation walk. Each step in the linked-list traversal
	// performs an unindexed JSON-extract scan in SQLite (and a similarly
	// expensive lookup in Postgres / Mongo) — letting an attacker chain
	// hundreds of synthetic `previous_response_id` values amplifies an
	// admin's "open conversation" click into hundreds of full-table scans.
	// 50 entries is more than enough for any realistic conversation.
	const maxConversationLimit = 50
	if limit <= 0 {
		return 40
	}
