...
func TestSQLiteStore_WriteBatch_ExactBatchBoundary(t *testing.T) {
	...
	// Test with exactly sqliteMaxRows entries
	numEntries := sqliteMaxRows
	entries := make([]*LogEntry, numEntries)
	for i := range numEntries {
		entries[i] = &LogEntry{
	...
	// Test with sqliteMaxRows + 1 entries - should require 2 batches
	entries = make([]*LogEntry, sqliteMaxRows+1)
	for i := 0; i <= sqliteMaxRows; i++ {
		entries[i] = &LogEntry{
	...
	expectedTotal := numEntries + sqliteMaxRows + 1
