...
// encodeLogData marshals the Data field to JSON for SQL storage.
// Returns nil if data is nil, or "{}" if marshaling fails.
// This is used by PostgreSQL and SQLite stores.
func encodeLogData(data *LogData, entryID string) []byte {
	if data == nil {
		return nil
	}
...
func cleanCacheType(value string) string {
