...
// BroadcastLive publishes an entry to live SSE subscribers without writing to storage.
// This allows streaming entries to appear in the UI immediately when streaming starts,
// before the full entry is available (e.g., before stream completion).
func (l *Logger) BroadcastLive(entry *LogEntry) {
	if entry == nil {
		return
	}
	l.broadcaster.Publish(entry)
...
func (l *Logger) SubscribeLive(bufferSize int) *LogSubscriber {
	return l.broadcaster.Subscribe(bufferSize)
}

func (l *Logger) UnsubscribeLive(id string) {
	l.broadcaster.Unsubscribe(id)
}

func (l *Logger) LiveSubscriberCount() int {
	return l.broadcaster.Len()
}
...
func (l *NoopLogger) BroadcastLive(_ *LogEntry) {}

func (l *NoopLogger) SubscribeLive(_ int) *LogSubscriber {
	ch := make(chan *LogEntry, 1)
	close(ch)
	return &LogSubscriber{ID: "noop", Entries: ch}
}

func (l *NoopLogger) UnsubscribeLive(_ string) {}

func (l *NoopLogger) LiveSubscriberCount() int { return 0 }

// LoggerInterface defines the interface for loggers (both real and noop)
type LoggerInterface interface {
	Write(entry *LogEntry)
	BroadcastLive(entry *LogEntry)
	Config() Config
	Close() error
	SubscribeLive(bufferSize int) *LogSubscriber
	UnsubscribeLive(id string)
