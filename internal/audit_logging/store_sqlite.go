...
const (
	maxBindParams    = 999
	colsPerEntry    = 21
	sqliteMaxRows = maxBindParams / colsPerEntry // 49 entries
)
...
func NewSQLiteStore(db *sql.DB, retentionDays int) (*SQLiteStore, error) {
	...
	if err := renameSQLiteCol(db, auditLogTableName, "model", "requested_model"); err != nil {
...
func (s *SQLiteStore) WriteBatch(ctx context.Context, entries []*LogEntry) error {
	...
	for i := 0; i < len(entries); i += sqliteMaxRows {
		end := min(i+sqliteMaxRows, len(entries))
		chunk := entries[i:end]

		// Build batch insert query for this chunk
		placeholders := make([]string, len(chunk))
		values := make([]any, 0, len(chunk)*colsPerEntry)

		for j, e := range chunk {
			placeholders[j] = "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"

			dataJSON := encodeLogData(e.Data, e.ID)
			...
			if cacheType := cleanCacheType(e.CacheType); cacheType != "" {
		...
		if err != nil {
			return fmt.Errorf("failed to insert audit logs batch %d: %w", i/sqliteMaxRows, err)
		}
	}
...
func renameSQLiteCol(db *sql.DB, tableName, from, to string) error {
	if db == nil {
		return nil
	}
	fromExists, err := sqliteColExists(db, tableName, from)
	if err != nil || !fromExists {
		return err
	}
	toExists, err := sqliteColExists(db, tableName, to)
	if err != nil || toExists {
		return err
	}
...
func sqliteColExists(db *sql.DB, tableName, columnName string) (bool, error) {
