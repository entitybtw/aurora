...
type mongoAuthKeyDocument struct {
	...
	DeniedModels     []string   `bson:"denied_models,omitempty"`
	ProviderPoolID   *string    `bson:"provider_pool_id,omitempty"`
	RateLimits       RateLimits `bson:"rate_limits,omitempty"`
...
func (s *MongoDBStore) Create(ctx context.Context, key AuthKey) error {
	_, err := s.collection.InsertOne(ctx, mongoAuthKeyDocument{
		...
		TenantID:         tenantIDOrDefault(key.TenantID),
		AllowedProviders: key.AllowedProviders,
		AllowedModels:    key.AllowedModels,
		DeniedModels:     key.DeniedModels,
		ProviderPoolID:   key.ProviderPoolID,
...
func authKeyFromMongo(doc mongoAuthKeyDocument) AuthKey {
	return AuthKey{
		...
		TenantID:         tenantIDOrDefault(doc.TenantID),
		AllowedProviders: doc.AllowedProviders,
		AllowedModels:    doc.AllowedModels,
		DeniedModels:     doc.DeniedModels,
		ProviderPoolID:   doc.ProviderPoolID,
