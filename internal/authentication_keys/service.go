...
// GetView returns the admin-facing view for one auth key, or nil if missing.
func (s *Service) GetView(id string) *View {
	if s == nil {
		return nil
	}
	id = normalizeID(id)
...
func (s *Service) Create(ctx context.Context, input CreateInput) (*IssuedKey, error) {
	...
	key := AuthKey{
		...
		ProviderPoolID:   normalized.ProviderPoolID,
