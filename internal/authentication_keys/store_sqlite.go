...
func NewSQLiteStore(db *sql.DB) (*SQLiteStore, error) {
	...
	migrations := []string{
		...
		`ALTER TABLE auth_keys ADD COLUMN requests_per_minute INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE auth_keys ADD COLUMN requests_per_day INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE auth_keys ADD COLUMN tokens_per_minute INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE auth_keys ADD COLUMN tokens_per_day INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE auth_keys ADD COLUMN allowed_providers TEXT`,
		`ALTER TABLE auth_keys ADD COLUMN allowed_models TEXT`,
		`ALTER TABLE auth_keys ADD COLUMN denied_models TEXT`,
		`ALTER TABLE auth_keys ADD COLUMN provider_pool_id TEXT`,
		`ALTER TABLE auth_keys ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default'`,
	}
...
func (s *SQLiteStore) List(ctx context.Context) ([]AuthKey, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, name, description, user_path, tenant_id, requests_per_minute, requests_per_day, tokens_per_minute, tokens_per_day, allowed_providers, allowed_models, denied_models, provider_pool_id, redacted_value, secret_hash, enabled, expires_at, deactivated_at, created_at, updated_at
		FROM auth_keys
...
func (s *SQLiteStore) Create(ctx context.Context, key AuthKey) error {
	allowedProvidersJSON, _ := json.Marshal(key.AllowedProviders)
	allowedModelsJSON, _ := json.Marshal(key.AllowedModels)
	deniedModelsJSON, _ := json.Marshal(key.DeniedModels)

	_, err := s.db.ExecContext(ctx, `
		INSERT INTO auth_keys (id, name, description, user_path, tenant_id, requests_per_minute, requests_per_day, tokens_per_minute, tokens_per_day, allowed_providers, allowed_models, denied_models, provider_pool_id, redacted_value, secret_hash, enabled, expires_at, deactivated_at, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, key.ID, key.Name, key.Description, nullableString(key.UserPath), tenantIDOrDefault(key.TenantID), key.RateLimits.RequestsPerMinute, key.RateLimits.RequestsPerDay, key.RateLimits.TokensPerMinute, key.RateLimits.TokensPerDay, string(allowedProvidersJSON), string(allowedModelsJSON), string(deniedModelsJSON), nullableStringPtr(key.ProviderPoolID), key.RedactedValue, key.SecretHash, boolToSQLite(key.Enabled), unixOrNil(key.ExpiresAt), unixOrNil(key.DeactivatedAt), key.CreatedAt.Unix(), key.UpdatedAt.Unix())
	if err != nil {
...
func scanSQLiteAuthKey(scanner authKeyScanner) (AuthKey, error) {
	...
	var allowedProviders sql.NullString
	var allowedModels sql.NullString
	var deniedModels sql.NullString
	var providerPoolID sql.NullString
	...
	key.TenantID = tenantIDOrDefault(tenantID)
	...
	if allowedProviders.Valid && allowedProviders.String != "" {
		_ = json.Unmarshal([]byte(allowedProviders.String), &key.AllowedProviders)
	}
	if allowedModels.Valid && allowedModels.String != "" {
		_ = json.Unmarshal([]byte(allowedModels.String), &key.AllowedModels)
	}
	if deniedModels.Valid && deniedModels.String != "" {
		_ = json.Unmarshal([]byte(deniedModels.String), &key.DeniedModels)
	}
	if providerPoolID.Valid {
		str := strings.TrimSpace(providerPoolID.String)
		if str != "" {
			key.ProviderPoolID = &str
		}
	}
...
func tenantIDOrDefault(value string) string {
	value = strings.TrimSpace(value)
	if value == "" {
		return "default"
	}
