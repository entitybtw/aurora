...
func TestServiceCreatePersistsRateLimitsAndReturnsThemOnAuthenticate(t *testing.T) {
	service, err := NewService(newTestStore())
	if err != nil {
		t.Fatalf("NewService() error = %v", err)
	}
	...
	issued, err := service.Create(context.Background(), CreateInput{Name: "limited", RateLimits: limits})
	if err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	if issued.RateLimits != limits {
		t.Fatalf("issued.RateLimits = %#v, want %#v", issued.RateLimits, limits)
	}
	...
	if authenticated.RateLimits != limits {
		t.Fatalf("Authenticate().RateLimits = %#v, want %#v", authenticated.RateLimits, limits)
	}
}

func TestServiceCreateRejectsNegativeRateLimits(t *testing.T) {
	service, err := NewService(newTestStore())
	if err != nil {
		t.Fatalf("NewService() error = %v", err)
	}
