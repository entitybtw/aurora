...
func TestBuildStoredPath(t *testing.T) {
	tests := []struct {
		name     string
		scopeKey string
		userPath string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := buildStoredPath(tt.scopeKey, tt.userPath); got != tt.want {
				t.Fatalf("buildStoredPath(%q, %q) = %q, want %q", tt.scopeKey, tt.userPath, got, tt.want)
			}
		})
	}
}
