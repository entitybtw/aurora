...
type CompiledWorkflow struct {
	...
	InputPipeline  *guardrails.Pipeline
	OutputPipeline *guardrails.Pipeline
...
func NewService(store Store, compiler Compiler) (*Service, error) {
	...
	service.current.Store(wfSnapshot{
		paths:              map[string]*CompiledWorkflow{},
		providers:          map[string]*CompiledWorkflow{},
		providerPaths:      map[string]map[string]*CompiledWorkflow{},
		providerModels:     map[string]map[string]*CompiledWorkflow{},
...
func (s *Service) Refresh(ctx context.Context) error {
	...
	return s.refreshUnderLock(ctx)
}

func (s *Service) refreshUnderLock(ctx context.Context) error {
	...
	for _, version := range versions {
		scope, scopeKey, err := cleanScope(version.Scope)
		if err != nil {
...
func (s *Service) EnsureDefaultGlobal(ctx context.Context, input CreateInput) error {
	input.Managed = true
	normalized, _, workflowHash, err := cleanInput(input)
	if err != nil {
		return err
	}
	if normalized.Scope.Provider != "" || normalized.Scope.Model != "" || normalized.Scope.UserPath != "" {
		return newWFError("default workflow must use global scope", nil)
	}
	...
	previewCompiled, err := s.validateNewVersion(normalized, "global", workflowHash)
	if err != nil {
		return err
	}
	s.refreshMu.Lock()
	...
	if version == nil {
		if s.snapshot().global == nil {
			if err := s.refreshUnderLock(ctx); err != nil {
				return err
			}
		}
		return nil
	...
	s.saveActivatedLocked(getCompiledForVersion(previewCompiled, *version))
	return nil
}
...
func (s *Service) Create(ctx context.Context, input CreateInput) (*Version, error) {
	...
	normalized, scopeKey, workflowHash, err := cleanInput(input)
	if err != nil {
		return nil, err
	}
	previewCompiled, err := s.validateNewVersion(normalized, scopeKey, workflowHash)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Deactivate(ctx context.Context, id string) error {
	...
	if scope.Provider == "" && scope.Model == "" && scope.UserPath == "" {
		return newWFError("cannot deactivate the global workflow", nil)
	}
	if !version.Active {
		return newWFError("workflow is already inactive", nil)
	}
	...
	s.saveDeactivatedLocked(*version)
	return nil
}
...
func (s *Service) ListViews(ctx context.Context) ([]View, error) {
	...
	sort.SliceStable(views, func(i, j int) bool {
		...
		if leftSpecificity, rightSpecificity := viewSpecificity(left.ScopeType), viewSpecificity(right.ScopeType); leftSpecificity != rightSpecificity {
...
func (s *Service) Match(selector core.WorkflowSelector) (*core.ResolvedWorkflowPolicy, error) {
	compiled, err := s.findCompiledMatch(selector)
...
// OutputPipelineForContext resolves the active response guardrails pipeline.
func (s *Service) OutputPipelineForContext(ctx context.Context) *guardrails.Pipeline {
	if s == nil || ctx == nil {
		return nil
	}
	workflow := core.GetWorkflow(ctx)
	...
	return s.OutputPipelineForWorkflow(workflow)
}
...
func (s *Service) PipelineForWorkflow(workflow *core.Workflow) *guardrails.Pipeline {
	...
	if compiled.InputPipeline != nil {
		return compiled.InputPipeline
	}
	return compiled.Pipeline
}

// OutputPipelineForWorkflow resolves the active output guardrails pipeline for one workflow.
func (s *Service) OutputPipelineForWorkflow(workflow *core.Workflow) *guardrails.Pipeline {
	if s == nil || workflow == nil || workflow.Policy == nil || !workflow.GuardrailsEnabled() {
		return nil
	}
	versionID := strings.TrimSpace(workflow.Policy.VersionID)
	...
	return compiled.OutputPipeline
}
...
func (s *Service) findCompiledMatch(selector core.WorkflowSelector) (*CompiledWorkflow, error) {
	if s == nil {
		return nil, nil
	}
...
func (s *Service) validateNewVersion(input CreateInput, scopeKey, workflowHash string) (*CompiledWorkflow, error) {
	version := Version{
		ID:           "preview",
		Scope:        input.Scope,
		ScopeKey:     scopeKey,
	...
	compiled, err := s.compiler.Compile(version)
	if err != nil {
		return nil, newWFError(err.Error(), err)
	}
	if compiled == nil || compiled.Policy == nil {
		return nil, newWFError("compiled workflow is empty or missing policy", nil)
	}
	return compiled, nil
}

func (s *Service) buildView(version Version) (View, error) {
	scope, scopeKey, err := cleanScope(version.Scope)
	if err != nil {
	...
	view.ScopeType = scopeTypeName(scope)
	view.ScopeDisplay = scopeDisplayName(scope)
	view.EffectiveFeatures = compiled.Policy.Features
	view.GuardrailsHash = compiled.Policy.GuardrailsHash
	return view, nil
}

func newErrorView(version Version, err error) View {
	scope := Scope{
	...
	view := NewViewFromVersion(version)
	view.ScopeType = scopeTypeRaw(scope)
	view.ScopeDisplay = scopeDisplayRaw(scope)
	view.CompileError = err.Error()
	return view
}

func scopeTypeRaw(scope Scope) string {
	switch {
...
func scopeDisplayRaw(scope Scope) string {
...
func viewSpecificity(scopeType string) int {
...
func (s *Service) snapshot() wfSnapshot {
	if s == nil {
		return wfSnapshot{
			paths:              map[string]*CompiledWorkflow{},
			providers:          map[string]*CompiledWorkflow{},
	...
	if current, ok := s.current.Load().(wfSnapshot); ok {
		return current
	}
	return wfSnapshot{
		paths:              map[string]*CompiledWorkflow{},
...
func copySnapshot(current wfSnapshot) wfSnapshot {
	next := wfSnapshot{
...
func getCompiledForVersion(compiled *CompiledWorkflow, version Version) *CompiledWorkflow {
	if compiled == nil {
		return nil
	}
...
func (s *Service) saveActivatedLocked(compiled *CompiledWorkflow) {
	if s == nil || compiled == nil {
		return
	}
	next := copySnapshot(s.snapshot())
...
func (s *Service) saveDeactivatedLocked(version Version) {
	if s == nil {
		return
	}
	next := copySnapshot(s.snapshot())
