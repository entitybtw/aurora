...
func NewSQLiteStore(db *sql.DB) (*SQLiteStore, error) {
	...
	if _, err := db.Exec(`ALTER TABLE workflow_versions ADD COLUMN scope_user_path TEXT`); err != nil && !isDuplicateColumnErr(err) {
		return nil, fmt.Errorf("initialize workflow versions table: %w", err)
	}
	if _, err := db.Exec(`ALTER TABLE workflow_versions ADD COLUMN managed_default INTEGER NOT NULL DEFAULT 0`); err != nil && !isDuplicateColumnErr(err) {
		return nil, fmt.Errorf("initialize workflow versions table: %w", err)
	}
	if _, err := db.Exec(`
...
func (s *SQLiteStore) Create(ctx context.Context, input CreateInput) (*Version, error) {
	...
	`,
		version.ID,
		asNullable(version.Scope.Provider),
		asNullable(version.Scope.Model),
		asNullable(version.Scope.UserPath),
		version.ScopeKey,
		version.Version,
		boolToInt(version.Active),
		boolToInt(version.Managed),
		version.Name,
		version.Description,
		string(payloadJSON),
...
func (s *SQLiteStore) EnsureManagedDefaultGlobal(ctx context.Context, input CreateInput, workflowHash string) (*Version, error) {
	...
	row := conn.QueryRowContext(ctx, `
		...
		LIMIT 1
	`)
	activeVersion, err := scanVersion(row)
	hasActive := true
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			hasActive = false
...
func scanVersion(scanner interface {
	...
	version.Scope = Scope{
		...
		UserPath: buildStoredPath(version.ScopeKey, scopeUserPath.String),
	}
...
func asNullable(value string) any {
	if value == "" {
		return nil
	}
	return value
