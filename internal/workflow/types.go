...
func (f FeatureFlags) normalize() FeatureFlags {
	if f.Budget == nil {
		budgetEnabled := true
...
func (f FeatureFlags) activeFeatures() core.WorkflowFeatures {
	f = f.normalize()
...
func cleanScope(scope Scope) (Scope, string, error) {
	...
	if err != nil {
		return Scope{}, "", newWFError("invalid scope_user_path", err)
	}
	scope.UserPath = userPath
	if scope.Provider == "" && scope.Model != "" {
		return Scope{}, "", newWFError("scope_model requires scope_provider_name", nil)
	}
	if strings.Contains(scope.Provider, ":") || strings.Contains(scope.Model, ":") || strings.Contains(scope.UserPath, ":") {
		return Scope{}, "", newWFError("scope fields cannot contain ':'", nil)
	}
	return scope, buildScopeKey(scope), nil
}

func buildScopeKey(scope Scope) string {
	switch {
...
func cleanPayload(payload Payload) (Payload, string, error) {
	if payload.SchemaVersion == 0 {
		payload.SchemaVersion = currentVersion
	}
	if payload.SchemaVersion != currentVersion {
	...
	for i, guardrail := range payload.Guardrails {
		...
		if guardrail.Ref == "" {
			return Payload{}, "", newWFError("guardrail ref is required", nil)
		}
		if _, exists := seenRefs[guardrail.Ref]; exists {
			return Payload{}, "", newWFError("duplicate guardrail ref: "+guardrail.Ref, nil)
		}
	...
	if err != nil {
		return Payload{}, "", newWFError("marshal workflow payload", err)
	}
	sum := sha256.Sum256(raw)
	return payload, hex.EncodeToString(sum[:]), nil
}

func cleanInput(input CreateInput) (CreateInput, string, string, error) {
	scope, scopeKey, err := cleanScope(input.Scope)
	if err != nil {
		return CreateInput{}, "", "", err
	}

	payload, workflowHash, err := cleanPayload(input.Payload)
	if err != nil {
		return CreateInput{}, "", "", err
	}
	...
	if input.Managed && (scope.Provider != "" || scope.Model != "" || scope.UserPath != "") {
		return CreateInput{}, "", "", newWFError("managed default workflow must use global scope", nil)
	}
	if !input.Managed &&
		input.Name == ManagedDefaultGlobalName &&
		input.Description == ManagedDefaultGlobalDescription {
		return CreateInput{}, "", "", newWFError("managed default workflow name/description is reserved", nil)
	}
