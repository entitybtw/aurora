...
func New(ctx context.Context, cfg *config.Config, compiler Compiler, refreshInterval time.Duration) (*Result, error) {
	...
	result, err := buildResult(ctx, storeConn, compiler, refreshInterval)
	if err != nil {
		_ = storeConn.Close()
		return nil, err
	}
...
func NewWithSharedStorage(ctx context.Context, shared storage.Storage, compiler Compiler, refreshInterval time.Duration) (*Result, error) {
	...
	return buildResult(ctx, shared, compiler, refreshInterval)
}

func buildResult(ctx context.Context, storeConn storage.Storage, compiler Compiler, refreshInterval time.Duration) (*Result, error) {
	store, err := openStore(ctx, storeConn)
	if err != nil {
		return nil, err
	}
...
func openStore(ctx context.Context, store storage.Storage) (Store, error) {
