...
func (s *MongoDBStore) ListActive(ctx context.Context) ([]Version, error) {
	...
	for cursor.Next(ctx) {
		...
		versions = append(versions, toVersion(doc))
	}
...
func (s *MongoDBStore) Create(ctx context.Context, input CreateInput) (*Version, error) {
	...
	result, err := session.WithTransaction(ctx, func(sessionCtx context.Context) (any, error) {
		...
		if err := s.insertVersionRow(sessionCtx, version); err != nil {
...
func (s *MongoDBStore) ensureDefaultVersion(ctx context.Context, input CreateInput, workflowHash string) (*Version, error) {
	session, err := s.collection.Database().Client().StartSession()
	if err != nil {
		return nil, fmt.Errorf("start workflow session: %w", err)
	}
...
func (s *MongoDBStore) insertVersionRow(ctx context.Context, version *Version) error {
	if version == nil {
		return fmt.Errorf("version is required")
	}
	_, err := s.collection.InsertOne(ctx, versionDoc{
		ID:              version.ID,
...
func toVersion(doc versionDoc) Version {
	return Version{
		ID: doc.ID,
		Scope: Scope{
