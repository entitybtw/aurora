...
func NewSQLiteStore(db *sql.DB) (*SQLiteStore, error) {
	...
	if _, err := db.Exec(`ALTER TABLE model_overrides ADD COLUMN enabled INTEGER NOT NULL DEFAULT 1`); err != nil && !isSQLiteDuplicateColumnError(err) {
		return nil, fmt.Errorf("failed to migrate model_overrides enabled column: %w", err)
	}
...
func (s *SQLiteStore) List(ctx context.Context) ([]Override, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT selector, provider_name, model, enabled, user_paths, created_at, updated_at
...
func (s *SQLiteStore) Upsert(ctx context.Context, override Override) error {
	...
	_, err = s.db.ExecContext(ctx, `
		INSERT INTO model_overrides (
			selector, provider_name, model, enabled, user_paths, created_at, updated_at
		)
		VALUES (?, ?, ?, ?, ?, ?, ?)
	...
	`,
		...
		override.Model,
		boolToSQLiteInt(override.EnabledValue()),
		pathsJSON,
