...
func TestExpandString(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "${AURORA_MASTER_KEY:-}",
			envVars:  map[string]string{"AURORA_MASTER_KEY": "secret-key"},
			expected: "secret-key",
		},
		{
...
func TestApplyEnvOverrides(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "AURORA_MASTER_KEY override",
			envVars: map[string]string{"AURORA_MASTER_KEY": "my-secret"},
			check: func(t *testing.T, cfg *Config) {
