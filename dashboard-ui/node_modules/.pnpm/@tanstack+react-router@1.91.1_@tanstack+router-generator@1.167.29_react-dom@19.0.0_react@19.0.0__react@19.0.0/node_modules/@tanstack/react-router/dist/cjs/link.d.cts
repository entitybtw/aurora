...
import { Constrain, ConstrainLiteral, Expand, MakeDifferenceOptional, NoInfer, NonNullableUpdater, PickRequired, Updater, WithoutEmpty } from './utils.cjs';
...
export type SearchPaths<TRouter extends AnyRouter, TPrefix extends string, TPaths = FindDescendantPaths<TRouter, TPrefix>> = TPaths extends `${TPrefix}${infer TRest}` ? TRest : never;
...
export type RelativeToCurrentPathAutoComplete<TRouter extends AnyRouter, TFrom extends string, TTo extends string, TResolvedPath extends string = RemoveTrailingSlashes<ResolveRelativePath<TFrom, TTo>>> = SearchRelativePathAutoComplete<TRouter, TTo, TResolvedPath> | CurrentPath<TrailingSlashOptionByRouter<TRouter>>;
export type AbsolutePathAutoComplete<TRouter extends AnyRouter, TFrom extends string> = (string extends TFrom ? CurrentPath<TrailingSlashOptionByRouter<TRouter>> : TFrom extends `/` ? never : FindDescendantPaths<TRouter, TFrom> extends never ? never : CurrentPath<TrailingSlashOptionByRouter<TRouter>>) | (string extends TFrom ? ParentPath<TrailingSlashOptionByRouter<TRouter>> : TFrom extends `/` ? never : ParentPath<TrailingSlashOptionByRouter<TRouter>>) | RouteToPath<TRouter> | (TFrom extends '/' ? never : string extends TFrom ? never : RemoveLeadingSlashes<SearchPaths<TRouter, TFrom>>);
...
type RemoveLastSegment<T extends string, TAcc extends string = ''> = T extends `${infer TSegment}/${infer TRest}` ? TRest & `${string}/${string}` extends never ? `${TAcc}${TSegment}` : RemoveLastSegment<TRest, `${TAcc}${TSegment}/`> : TAcc;
export type ResolveCurrentPath<TFrom, TTo> = TTo extends '.' ? TFrom : TTo extends './' ? AddTrailingSlash<TFrom> : TTo & `./${string}` extends never ? never : TTo extends `./${infer TRest}` ? ResolveRelativePath<TFrom, TRest> : never;
export type ResolveParentPath<TFrom extends string, TTo> = TTo extends '../' ? RemoveLastSegment<TFrom> : TTo & `..${string}` extends never ? never : TTo extends `..${infer ToRest}` ? ResolveRelativePath<RemoveLastSegment<TFrom>, RemoveLeadingSlashes<ToRest>> : never;
export type ResolveRelativePath<TFrom, TTo = '.'> = string extends TFrom ? TTo : string extends TTo ? TFrom : undefined extends TTo ? TFrom : TFrom extends string ? TTo extends string ? TTo & `..${string}` extends never ? TTo & `.${string}` extends never ? TTo & `/${string}` extends never ? AddLeadingSlash<JoinPath<TFrom, TTo>> : TTo : ResolveCurrentPath<TFrom, TTo> : ResolveParentPath<TFrom, TTo> : never : never;
